...
const int PulseWire = A0; // Pulse sensörümüzü bağlamış olduğumuz Analog pinini belirliyoruz.
...
void setup() {
  ...
  pinMode(lcd_ledi, OUTPUT); // LCD arka plan aydınlatma pinini çıkış olarak ayarlıyoruz.
...
void loop() {
  ...
  if (pulseSensor.sawStartOfBeat()) { //Eğer nabız algılarsak
    ...
    myGLCD.setFont(BigNumbers); //Büyük sayı fontuyla yazmayı sağlıyoruz.
  ...
  delay(20); //20 milisaniye bekletiyoruz.
}
